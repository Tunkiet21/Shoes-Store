                ...
                checkAvailable(receiverEmail, new OnCheckAvailableListener() {
                    ...
                    public void onCheckAvailable(boolean isAvailable) {
                        if (isAvailable) {
                            // Email đã tồn tại
                            init();
                            showToast("Email đã tồn tại, vui lòng nhập email khác!");
                        } else {
                            // Email chưa tồn tại
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (!countDownTextView.getText().toString().isEmpty()){
                    if (Integer.valueOf(inputOTP) == otp) {
                        ...
                        init();
                        showToast("OTP không đúng vui lòng nhập lại");
                    }
                }
                else {
                    dialog.cancel();
                    init();
                    showToast("OTP đã hết hạn!");
                }
            }
        });
...
//                preferenceManager.putString(Constants.KEY_IMAGE, encodedImage);
    ...
    private Boolean isValidSignUpDetails() {
        ...
        if (binding.inputName.getText().toString().trim().isEmpty()) {
            ...
            showToast("Hãy nhập đúng định dạng email");
            return false;
        } else if (password.isEmpty()) {
            showToast("Hãy nhập mật khẩu!");
            return false;
        } else if (password.length() < 10) {
            showToast("Mật khẩu tối thiểu 10 ký tự");
            return false;
        } else if (password.length() > 20) {
            showToast("Mật khẩu không được nhập quá 20 ký tự");
            return false;
        } else if (!password.matches("^(?=.*[a-zA-Z])(?=.*\\d).+$")) {
            // Kiểm tra mật khẩu phải có ít nhất 1 chữ cái và 1 số
            showToast("Mật khẩu phải chứa ít nhật 1 chữ cái và 1 số");
            return false;
        } else if (password.matches(".*[^a-zA-Z\\d].*")) {
            ...
            return false;
        } else if (!password.equals(binding.inputConfirmPassword.getText().toString())) {
