...
public class CreateProductActivity extends AppCompatActivity {
    ...
    private void loadCategories() {
        databaseReference.child(Constants.KEY_COLLECTION_CATEGORY).addValueEventListener(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(CreateProductActivity.this, "Lỗi khi tải danh mục: " + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == PICK_IMAGE_CAMERA || requestCode == PICK_IMAGE_GALLERY) {
                ...
                try {
                    ...
                    // Lấy tỷ lệ của ảnh gốc
                    int width = bitmap.getWidth();
                    int height = bitmap.getHeight();
                    float aspectRatio = (float) width / height;

                    // Đặt lại kích thước dựa trên tỷ lệ
                    int newWidth = 500;
            ...
            fileReference.putFile(imageUri)
                    ...
                    .addOnFailureListener(new OnFailureListener() {
                        ...
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(CreateProductActivity.this, "Lỗi khi tải ảnh: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    });
        } else {
    ...
    private void saveProductToDatabase(String imageUrl) {
        ...
        if (!title.isEmpty() && !priceStr.isEmpty() && !oldPriceStr.isEmpty() && !description.isEmpty() && selectedCategoryId != null) {
            ...
            // Chuyển đổi chuỗi kích thước thành danh sách và kiểm tra tính hợp lệ của từng kích thước
            List<String> sizes = Arrays.asList(sizesStr.split("\\s*,\\s*"));
            for (String sizeStr : sizes) {
                try {
                    int size = Integer.parseInt(sizeStr);
                    if (size < 34 || size > 43) {
                        Toast.makeText(this, "Kích thước phải nằm trong khoảng từ 34 đến 43", Toast.LENGTH_SHORT).show();
                        return; // Dừng lại nếu kích thước không hợp lệ
                    }
                } catch (NumberFormatException e) {
                    Toast.makeText(this, "Kích thước không hợp lệ", Toast.LENGTH_SHORT).show();
                    return; // Dừng lại nếu không thể chuyển đổi chuỗi thành số
                }
            }
                ...
                databaseReference.child(Constants.KEY_COLLECTION_PRODUCTS).child(id).setValue(product)
                        .addOnCompleteListener(task -> {
                            if (task.isSuccessful()) {
                                Toast.makeText(CreateProductActivity.this, "Đã lưu sản phẩm", Toast.LENGTH_SHORT).show();
                                finish();
                            } else {
                                Toast.makeText(CreateProductActivity.this, "Lỗi khi lưu sản phẩm", Toast.LENGTH_SHORT).show();
                            }
                        });
            }
        } else {
